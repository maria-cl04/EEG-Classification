...
class ITSA:
    ...
    def adapt_loso_test_subject(
        ...
        # Recolour toward the global training reference G.
        # This is more stable than back-projecting a noisy 40-class rotation:
        # reference_G_ is the Riemannian mean of ALL recentered training
        # covariances, estimated from thousands of samples. It is the natural
        # centre of the training distribution and puts subject 1's filtered
        # signals in the same covariance space as training subjects, without
        # depending on per-class anchor estimates that are unreliable at 40
        # classes with N_test/2 samples per fold.
        #
        # Recentering-only (M^{-1/2}) is NOT sufficient: it whitens the signals,
        # removing all spatial structure. Training subjects are whitened AND
        # recoloured (W restores structure). Using identity for W leaves the
        # test signals in a completely different space — the transformer finds
        # them uninformative and produces chance-level accuracy every epoch.
        assert self.reference_G_ is not None, \
            "reference_G_ not found — was fit() called before adapt_loso_test_subject()?"

        G_target = to_spd_np(self.reference_G_, eps=self.subject_eps)
