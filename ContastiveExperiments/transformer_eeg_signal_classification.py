...
### NEW ###
# Instantiate the SupCon loss function once (it has no learnable parameters).
...
for epoch in range(1, opt.epochs + 1):
    ...
    for split in ("train", "val", "test"):
        ...
        for i, (input, target) in enumerate(loaders[split]):
            ...
            ### CHANGED ###
            # During training, request the projection vector from the model.
            # During val/test we do NOT need it — standard forward pass, no overhead.
            if split == "train" and opt.supcon:
                # return_proj=True → model returns (logits, proj)
                # proj is already L2-normalised (see transformer2.py Model.forward)
                output, proj = model(input, return_proj=True)

                # Cross-entropy loss (same as before)
                ce_loss = F.cross_entropy(output, target)

                # Supervised contrastive loss on the projected embeddings.
                # This term encourages same-class EEG trials to cluster together
                # in the projection space, learning more generalisable representations.
                sc_loss = supcon_loss_fn(proj, target)

                # Combined loss: CE dominates early in training; SupCon adds
                # a regularising pull that helps especially in multi-subject
                # and fine-tuning settings.
                loss = ce_loss + opt.lambda_supcon * sc_loss

            else:
                # Validation / test: standard forward pass, no projection needed.
                output = model(input)
