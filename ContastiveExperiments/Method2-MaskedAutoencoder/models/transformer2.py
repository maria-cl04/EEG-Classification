...
class Model(nn.Module):
    ...
    # -------------------------------------------------------------------
    # Internal helper: run the encoder and return the mean-pooled vector.
    # This is the "embedding" used for both the classifier and the
    # contrastive projection head.
    # -------------------------------------------------------------------
    def _encode(self, x: torch.Tensor, mask=None) -> torch.Tensor:
        """Return mean-pooled encoder output, shape (B, d_model)."""
        if x.dim() == 4:
            # (B, 1, 128, T) → (B, T, 128)
    ...
    ### NEW ###
    def get_embedding(self, x: torch.Tensor, mask=None) -> torch.Tensor:
        """
        Public method for extracting embeddings without computing logits.
        Used by the prototypical-network evaluation script.
        Returns: (B, d_model) tensor, no gradient computation assumed.
        """
