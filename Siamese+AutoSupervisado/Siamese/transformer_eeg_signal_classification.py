...
# Siamese / Triplet options
parser.add_argument('--siamese', default=False, action="store_true", help="Enable Siamese (triplet) training")
parser.add_argument('--lambda-siamese', default=0.05, type=float, help="Weight of the triplet loss")
parser.add_argument('--triplet-margin', default=1.0, type=float, help="Margin for online hard triplet loss")
    ...
    for split in ("train", "val", "test"):
        ...
        for i, (input, target) in enumerate(loaders[split]):
            ...
            if split == "train" and opt.simclr:
                # 1. Standard classification pass on clean data
                output = model(input)
                loss_ce = F.cross_entropy(output, target)

                # 2. Generate two independent augmented views
                view1 = augmenter.random_augment(input)
                view2 = augmenter.random_augment(input)

                # 3. Get L2-normalised projections for both views
                _, z_i = model(view1, return_proj=True)
                _, z_j = model(view2, return_proj=True)

                # 4. Compute self-supervised contrastive loss
                ...
                if i == 0:  # diagnostic printout once per epoch
                    print(f"  [ep{epoch} batch0] CE={loss_ce.item():.4f}  SimCLR={loss_simclr.item():.4f}")

            # --- SIAMESE (TRIPLET) BRANCH ---
            elif split == "train" and opt.siamese:
                # 1. Standard classification pass → logits for CE
                output = model(input)
                loss_ce = F.cross_entropy(output, target)

                # 2. Raw encoder embeddings → triplet loss (Euclidean space)
            ...
            # --- STANDARD BRANCH (baseline, val, test) ---
            else:
                output = model(input)
